...
export const FooterRef = Node.create({
  ...
  addExtensions() {
    const FootnoteRegistry = Node.create({
      ...
      parseHTML() {
        return [
          {
            tag: 'footer.footnotes[aria-label="Footnotes"]',
          },
        ];
      },
    ...
    return {
      insertFooterRef: () => ({ tr, editor, dispatch }) => {
        ...
        dialogManager.show((content) => {
          ...
          console.log(nextNumber, `[${nextNumber}]`, `#bk${nextNumber}`);
  ...
  addKeyboardShortcuts() {
    return {
      "Shift-Alt-k": () => this.editor.commands.insertFooterRef(),
    };
  },
